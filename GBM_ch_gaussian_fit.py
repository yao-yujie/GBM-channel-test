...
databasedir='/home/yujie/data/数据文件/data'
...
class GRB:
	...
	def ch_rawlc(self,lcbinwidth=0.1):
		fig, axes= plt.subplots(11,1,figsize=(10, 14),sharex='col',sharey=False)
		channel1=11
		channel2=56
		channel3=63
		channel=np.arange(channel1,channel2+5,5)
		...
		for i in range(ch_l):			
			if i<ch_l-1:
				...
				time=data.field(0)-trigtime
				ch=data.field(1)			
				goodindex=(ch>=channel[i]) & (ch<=channel[i+1])  
				time=time[goodindex]
				ebound=hdu['EBOUNDS'].data
				emin=ebound.field(1)
				emin=emin[ch1:ch2+1]
				...
				histvalue, histbin =np.histogram(time,bins=self.tbins)
				plotrate=histvalue/lcbinwidth
				plotrate=np.concatenate(([plotrate[0]],plotrate))
				axes[i].plot(histbin,plotrate,linestyle='steps')
				axes[i].text(0.8,0.88,str(round(emin[channel[i]],1))+'-'+str(round(emin[channel[i+1]],1))+ \
																' keV',fontsize=10)
				...
				time=data.field(0)-trigtime
				ch=data.field(1)			
				goodindex=(ch>=channel[1]) & (ch<=channel[3]+1)  
				time=time[goodindex]
				ebound=hdu['EBOUNDS'].data
				emin=ebound.field(1)
				emin=emin[ch1:ch2+1]
				emax=ebound.field(2)
				emax=emax[channel[1]:channel[3]+1]
				histvalue, histbin =np.histogram(time,bins=self.tbins)
				plotrate=histvalue/lcbinwidth
				plotrate=np.concatenate(([plotrate[0]],plotrate))
				axes[i].plot(histbin,plotrate,linestyle='steps')
				axes[i].text(0.8,0.88,str(15-150)+ ' keV',fontsize=10)

						
				axes[i].set_xlim(-10,30)
				axes[i].set_ylim(0,1500)		

		plt.savefig('ch_rawlc.eps')
	...
	def gaussian_net_rate(self,sigma=3):
			...
			f=h5py.File(self.baseresultdir+'/base.h5',mode='r')		
			cNet=np.array([f['/'+Det[11]+'/ch'+str(ch)][()][2] \
											for ch in np.arange(11,13) ])			
			totalNet=np.sum(cNet,axis=0)				
			totalNet=np.concatenate(([totalNet[0]],totalNet))
			t0=self.tbins			
			plt.plot(t0,totalNet,linestyle='steps')
			def gaussian_fit(t0,a1,t1,l1):
				return a1*np.exp(-((t0-t1)**2/(2*l1**2)))
			x,y=optimize.curve_fit(gaussian_fit,t0,totalNet)
			print('x',x,'y',y)
			a1=x[0]
			t1=x[1]			
			l1=x[2]		
			y=gaussian_fit(t0,a1,t1,l1)
			print(y)
			plt.plot(t0,y,'r')	
			plt.xlim(-10,30)
			plt.ylim(0,300)
...
for n in range(1,nl):
	...
	#print('epeak',epeak)
	#grb.ch_rawlc(lcbinwidth=0.1)
	grb.gaussian_net_rate(sigma=3)	
	grb.ch_netlc(lcbinwidth=0.1)
